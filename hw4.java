...
public class hw4 {
	...
	public class MyDequeUsingStacks<T> {
		...
		// popLeft will pop from the left of the queue. If the left stack (sl) is empty,
		// items from the right stack (sr) are moved to left stack, then the top value is 
		// popped off the left stack and returned.
		public T popLeft () {
		...
		// popRight will pop from the right of the queue. If the right stack (sr) is empty,
		// items from the left stack (sl) are moved to right stack, then the top value is 
		// popped off of the right stack and returned.
		public T popRight () {
			if (isEmpty()) { throw new NoSuchElementException(); }
			if(sr.isEmpty()) {
				move(sl,sr);
		...
		private void move (ResizingArrayStack<T> from, ResizingArrayStack<T> to) {
			...
			// 
			// Your goal is to change this method by replacing the entire while-loop above 
			// so that overall performance of the main method improves, by altering 
			// the number of elements moved from "from" to "to".
			//
	...
	private static void correctnessTest () {
		MyDequeUsingStacks<Integer> d1 = new hw4().new MyDequeUsingStacks<> ();
		d1.pushLeft(0);
		d1.pushLeft(1);
		d1.pushLeft(2);
		d1.pushLeft(3);
	...
	private static double timeTrial(int N) {
		...
		return sw.elapsedTime () / NUM_TRIALS;
	}
